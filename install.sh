...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -q --no-check-certificate -N -O /usr/local/V2bX/V2bX-linux.zip https://github.com/leaderen/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        echo -e "开始安装 V2bX $1"
        wget -q --no-check-certificate -N -O /usr/local/V2bX/V2bX-linux.zip ${url}
        if [[ $? -ne 0 ]]; then
...
EOF
    ...
    curl -o /usr/bin/V2bX -Ls https://raw.githubusercontent.com/leaderen/V2bX-script/master/V2bX.sh
    ...
    if [[ $first_install == true ]]; then
        ...
        if [[ $if_generate == [Yy] ]]; then
            curl -o ./initconfig.sh -Ls https://raw.githubusercontent.com/leaderen/V2bX-script/master/initconfig.sh
